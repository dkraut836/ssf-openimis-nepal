from api_fhir.models import claim
...
from sosys.models import Insuree
...
from api_fhir_r4.serializers import PatientSerializer, LocationSerializer, LocationSiteSerializer, \
...
from sosys.serializers import InsureeSerializer
import datetime
...
class InsureeViewSet(BaseFHIRView, viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if identifier:
            queryset = queryset.filter(p_ssid=identifier)
        else:
            ...
            if claim_date is not None:
                ...
                has_claim_in_range = Claim.objects \
                    ...
                    .values("id")
                queryset = queryset.annotate(has_claim_in_range=Exists(has_claim_in_range)).filter(
    ...
    def get_queryset(self):
        ...
        # order_by('p_ssid').select_related('AddressId')
...
class LocationViewSet(BaseFHIRView, viewsets.ModelViewSet):
    ...
    def get_queryset(self, physicalType='area'):
        ...
        if physicalType == 'si':
            return HealthFacility.objects.select_related('location').select_related('sub_level').select_related(
                'legal_form')
        else:
...
class ContractViewSet(BaseFHIRView, mixins.RetrieveModelMixin, mixins.ListModelMixin, GenericViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        queryset = self.get_queryset().select_related('product').select_related('officer') \
            ...
            .prefetch_related(Prefetch('insuree_policies',
