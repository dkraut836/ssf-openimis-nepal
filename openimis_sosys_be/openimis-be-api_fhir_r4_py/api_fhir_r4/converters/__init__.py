...
class BaseFHIRConverter(ABC):
    ...
    @classmethod
    def build_fhir_pssid_identifier(cls, identifiers, imis_object):
        if hasattr(imis_object,'p_ssid') and imis_object.p_ssid is not None:
            identifier = cls.build_fhir_identifier(imis_object.p_ssid,
                                                   R4IdentifierConfig.get_fhir_identifier_type_system(),
                                                   R4IdentifierConfig.get_fhir_pssid_type_code())
            identifiers.append(identifier)
        else:
            raise FHIRRequestProcessException(['Cannot construct an identifier, the object has no uuid nor id: {}'])
