...
export function fetchBankPicker(mm) {  
  let payload = formatPageQuery("banks",
  null,
  mm.getRef("location.BankNamePicker.projection")
);
return graphql(payload, 'LOCATION_BANKS');
}

export function fetchBankTypePicker(mm,location) {
  console.log('accc000',location.bankDetails)  
  let payload = formatPageQuery("bankBranches",[`Bank_Bankid:"${location.bankDetails.length==0?"":location.bankDetails[0].bankId.BankId}"`],
  mm.getRef("location.BankTypePicker.projection")
);
console.log('lokk k', payload)
return graphql(payload, 'LOCATION_BANKTYPES');
}
...
export function fetchHealthFacility(mm, healthFacilityUuid, healthFacilityCode) {
  ...
  let projections = [
    ...
    "bankDetails{accountName,bankId{BankId,BankName},branchId{BranchId,BranchName},id,accountNumber}"
  ]
...
export function fetchLocations(levels, type, parent) {
  ...
  console.log('levels s', type)
  console.log('levels a', parent)
...
// for bank details
function formatBank(bankDetail) {
    return `{
      ${!!bankDetail.id ? `id: ${bankDetail.id}`:""}
      bankId: ${decodeId(bankDetail.bankId.BankId)}
      branchId: ${decodeId(bankDetail.branchId.BranchId)}
      accountName: "${bankDetail.accountName}"
      accountNumber: "${bankDetail.accountNumber}"
    }`
}

function formatBanks(bankDetails) {
  if (!bankDetails || !bankDetails.length) return "";
  

  return `bankDetails: [
    ${bankDetails.map(c => formatBank(c)).join('\n')}
  ]`
}
...
function formatHealthFacilityGQL(hf) {
  ...
  let paydatum = `
    ...
    ${formatBanks(hf.bankDetails)}
  `
  console.log("xxxzzzaaa",paydatum)
  return paydatum
}
