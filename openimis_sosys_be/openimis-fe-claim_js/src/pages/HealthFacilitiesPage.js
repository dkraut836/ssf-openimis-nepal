...
class HealthFacilitiesPage extends Component {
    ...
    onDoubleClick = (c, newTab = false) => {
        historyPush(this.props.modulesManager, this.props.history, "claim.route.claimEdit",[c.product.name,c.uuid], newTab)
    }

    onAdd = () => {
        historyPush(this.props.modulesManager, this.props.history, "claim.route.claimEdit",[this.props.claimType.name]);
    }

    canAdd = () => {
        if (!this.props.claimAdmin) return false
        if (!this.props.claimHealthFacility) return false
        if (!this.props.claimType) return false
        return true;
    }
