...
import { RIGHT_UPDATE, RIGHT_FEEDBACK, RIGHT_CLAIMREVIEW, RIGHT_PROCESS, RIGHT_FORWARD} from "../constants";
...
class ReviewsPage extends Component {
    ...
    canMarkSelectedForFeedback = selection => !!selection && selection.length &&
        ...
        selection.filter(s => s.status === 6 || s.status === 9).length === selection.length
    ...
    canProcessSelected = selection => !!selection && selection.length &&
                        selection.filter(s => (s.status === 6 || s.status === 9) && s.reviewStatus>=8).length === selection.length
    ...
    canForwardSelected = selection => !!selection && selection.length &&
                        selection.filter(s => s.status === 6 && s.reviewStatus >= 8 ).length === selection.length

    forwardSelected = selection => {
        this._labelMutation(selection,
            "ForwardClaim.mutationLabel",
            "ForwardClaims.mutationLabel",
            this.props.forward);
    }
    ...
    provideFeedback = c => historyPush(this.props.modulesManager, this.props.history, "claim.route.feedback", [c.product.name,c.uuid])
        ...
        <Grid container justify="flex-end" alignItems="center">
            <Grid item>
                <PublishedComponent
                    ...
                    readOnly={!this.props.rights.includes(RIGHT_UPDATE) || c.status !== 6 || c.status !== 9}
    ...
    review = (c, newTab = false) => historyPush(this.props.modulesManager, this.props.history, "claim.route.review", [c.product.name,c.uuid], newTab)
        ...
        <Grid container justify="flex-end" alignItems="center">
            <Grid item>
                <PublishedComponent
                    ...
                    readOnly={(!this.props.rights.includes(RIGHT_UPDATE) ||  c.reviewStatus >= 8) || c.status < 6 }
...
//        if (rights.includes(RIGHT_FORWARD)) {
            actions.push({ label: "Forward", enabled: this.canForwardSelected, action: this.forwardSelected });
//        }
        return (
