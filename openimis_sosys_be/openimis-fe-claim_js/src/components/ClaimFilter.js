...
import { selectClaimAdmin, selectHealthFacility, selectDistrict, selectRegion, selectClaimPicker,generateCode } from "../actions";
...
class Head extends Component {
    ...
    _healthFacilityFilter = v => {
        if (!!v) {
            this.props.generateCode(v.code)
            return {
                id: 'healthFacility',
                value: v,
    ...
    _claimPickeFilter = v => {
        if (!!v) {
            return {
                id: 'product',
                value: v,
                filter: `product_Code: "${v.code}"`
            }
        } else {
            return { id: 'product', value: null, filter: null }
        }
    }
    ...
    _onChangeClaimPicker = (v, s) => {
        this.props.onChangeFilters([
            this._claimPickeFilter(v),
        ]);
        this.setState({
            reset: this.state.reset + 1,
        });
        this.props.selectClaimPicker(v);
    }
    ...
    render() {
        ...
        return (
            <Grid container className={classes.form}>
                ...
                <ControlledField module="claim" id="ClaimFilter.product" field={
                    <Grid item xs={2} className={classes.item}>
                        <ClaimPicker
                            value={this._filterValue('product')}
                            withNull={true}
                            reset={this.state.reset}
                            onChange={this._onChangeClaimPicker}
                        />
                    </Grid>
                } />
...
const mapDispatchToProps = dispatch => {
    return bindActionCreators({ selectClaimAdmin, selectHealthFacility, selectDistrict, selectRegion,selectClaimPicker,generateCode }, dispatch);
};
    ...
    render() {
        ...
        return (
            <Grid container className={classes.form}>
                ...
                {/* <ControlledField module="claim" id="ClaimFilter.claimType" field={                    
                    <Grid item xs={2} className={classes.item}>
                        <ClaimPicker
                         value={(filters['claimType'] && filters['claimType']['value'])}
                         onChange={v => {
                            console.log('huuuuuuu ioo', v);
                            this.debouncedOnChangeFilter([
                                {
                                    id: 'claimType',
                                    value: v,
