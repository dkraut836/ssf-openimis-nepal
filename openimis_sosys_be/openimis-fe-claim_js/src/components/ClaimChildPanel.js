...
class ClaimChildPanel extends Component {
    ...
    initData = () => {
        let data = [];
        console.log('dataa is iss',this.props.edited[`${this.props.type}s`])
    ...
    componentDidMount() {
        ...
        console.log('dataa is', this.initData())
    }
    ...
    render() {
        const { intl, classes, edited, type, picker, forReview, fetchingPricelist, readOnly = false,rights } = this.props;
        ...
        let itemFormatters = [
            ...
            (i, idx) => {
                ...
                if(!!i.item && i.item.code.startsWith('ADJ')) isAdjustment = false; 
                else if(!!i.service && i.service.code.startsWith('ADJ')) isAdjustment = false; 
                return <AmountInput
                    readOnly={(!!forReview || readOnly || (this.fixedPricesAtEnter && isAdjustment))}
                    value={i.priceAsked}
        ...
        if (!!forReview || edited.status !== 2) {    
           preHeaders.push('', '');
            ...
            itemFormatters.push(
                (i, idx) => <PublishedComponent
                readOnly={((edited.status===6 && edited.reviewStatus!=8) || (edited.status===9 && edited.reviewStatus!=8))?false:readOnly}
                    // readOnly={readOnly}
