...
import {  Grid, FormGroup, Checkbox,FormControlLabel } from "@material-ui/core";
import _ from "lodash";
import ClaimAdminPicker from "../pickers/ClaimAdminPicker";
import { claimedAmount, approvedAmount } from "../helpers/amounts";
import { claimHealthFacilitySet, validateClaimCode,reClaimPicker, changeSpouseId, fetchReClaim,changeVisitDate } from "../actions";
...
class ClaimMasterPanelMedical extends FormPanel {
    ...
    onChangeReClaim=(e)=> {
    //     this.ChangeReClaim();
    //    this.updateAttribute("isReClaimed", this.state.isReClaimed?"false":"true");
        this.ChangeReClaim();
       this.updateAttribute([e.target.name], this.state.isReClaimed?"false":"true");
    }

    onChangeHeadClaim=(v)=>{
        this.updateAttribute("headClaim", v)
        // this.props.fetchReClaim(this.props.modulesManager,this.props.edited, v.code)
    }
    changeEventDate=(v)=>{
    ...
    onChangeRelation=(v)=>{
        if(v==='2' && !!this.props.edited.insuree){
            ...
            this.props.changeSpouseId(this.props.edited.insuree.chfId)
        }
    }

    render() {
        const { reclaims,intl, classes, edited, reset, readOnly = false, forReview, roReview = false, forFeedback } = this.props;
        ...
        return (
            <Grid container>
                 <ControlledField module="claim" id="Claim.subProduct" field={
                        <Grid item xs={3} className={classes.item}>
                            <AccidentialPlacePicker
                            claim={this.props.pickerr}
                            value={edited.subProduct}
                            onChange={(v, s) => this.updateAttribute("subProduct", v)}
                            label="Scheme"
                            readOnly={ro}
                            required={true}
                            />
...
{!!edited.code  &&
            <ControlledField module="claim" id="Claim.code" field={
                ...
                <ControlledField module="claim" id="Claim.visitType" field={
                    <Grid item xs={forFeedback || forReview ? 2 : 2} className={classes.item}>
                        <PublishedComponent
         ...
         <ControlledField module="claim" id="Claim.isReclaim" field={
                ...
                <ControlledField module="claim" id="Claim.headClaim" field={
                    <Grid item xs={this.state.isReClaimed || edited.isReclaim==="true"?2:0} className={classes.item}>
                        ...
                        readOnly={ro}
                        onChange={(v, s) => (this.onChangeHeadClaim(v))}
                        />
                    </Grid>
                } />
                }
...
const mapDispatchToProps = dispatch => {
    return bindActionCreators({ claimHealthFacilitySet, validateClaimCode,reClaimPicker,changeSpouseId,fetchReClaim,changeVisitDate }, dispatch);
};

export default withModulesManager(
    injectIntl(
        connect(mapStateToProps, mapDispatchToProps)(withTheme(withStyles(styles)(ClaimMasterPanelMedical))
        )
    )
)
