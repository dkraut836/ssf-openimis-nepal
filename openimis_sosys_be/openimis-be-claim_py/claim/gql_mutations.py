...
def call_SP(parameter,schemeId,claimType="C"):
    ...
    if claimType == "R":
        ...
        claim_code = generate_code(code_initial,"Null",schemeId,claimType)
...
def generate_code(claimCodeInitials,oldClaimCode,schemeId,claimType):
    ...
    sql = """\
            ...
            EXEC @return_value = [dbo].[uspClaimSequenceNo] @claimcodeinitials = '""" + claimCodeInitials + """',@oldclaimCode='"""+oldClaimCode+"""' ;      
    ...
    with connection.cursor() as cur:
        try:
            ...
            code = result_set
            if claimType == 'C':
                code = claimCodeInitials + str(currentYear.year)[1:] + str("{:07d}".format(result_set)) +str(schemeId)+ claimType
        finally:
            cur.close()
    return code
...
class CreateClaimMutation(OpenIMISMutation):
    ...
    def async_mutate(cls, user, **data):
        try:
            ...
            data['code']=call_SP(parameter,data['product_id'],claim_type)
...
class SaveClaimReviewMutation(OpenIMISMutation):
    ...
    def async_mutate(cls, user, **data):
        try:
            ...
            if data.get('scheme_app_id', None) == 2:
...
class ProcessClaimsMutation(OpenIMISMutation):
    ...
    def async_mutate(cls, user, **data):
        ...
        for claim_uuid in data["uuids"]:
            ...
            c_errors += PostClaimForPayment(user, claim_uuid)
            if c_errors:
                errors.append({
                    'title': claim.code,
                    'list': c_errors
                })
                claim.status = Claim.STATUS_RECOMMENDED
                claim.save()
        ...
        return errors
import os
def PostClaimForPayment(user,claim_uuid):
    # print('claim_uuis',claim_uuid)
    errors = []
    claim = Claim.objects \
        .filter(uuid=claim_uuid) \
        .first()
    ...
    a = {
            ...
            "ApprovedBy": claim.audit_user_id_process,
    ...
    print('claim.product.code',claim.product.code)
    ...
    if response.ok:
        ...
        errors += {
            'title': claim_uuid,
            'list': [{'message': _(
                "claim.validation.fail_to_push_payment") % {'id': claim_uuid}}]
        }
    return errors
