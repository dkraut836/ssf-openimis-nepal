...
from sosys.models import HFBankDetails,Bank,BankBranch
...
class HealthFacilityCodeInputType(graphene.String):
    ...
    def parse_literal(ast):
        result = graphene.String.parse_literal(ast)
        assert_string_length(result, 10)
        return result

class HealthFacilityBankDetailInputType(InputObjectType):
    id = graphene.Int(required=False, read_only=True)
...
class HealthFacilityInputType(OpenIMISMutation.Input):
    ...
    bankDetails = graphene.List(HealthFacilityBankDetailInputType, required=False)
...
# for bankDetails
def process_bankDetails(user, data_bankDetails, prev_hf_id,
                       hf_id, bankDetails):
    prev_bankDetails = [c.id for c in bankDetails.all()]
    from core.utils import TimeUtils
    print("ram2", data_bankDetails)
    print("ram3", prev_bankDetails)
    for bankDetail in data_bankDetails:
        bankDetail_id = bankDetail.pop('id') if 'id' in bankDetail else None
        print("ramji", bankDetail)
        if bankDetail_id:
            prev_bankDetails.remove(bankDetail_id)            
            print("ramji1", bankDetails)
            prev_bankDetail = bankDetails.filter(id=bankDetail_id, **bankDetail).first()
            print("ramji2", bankDetails)
            if not prev_bankDetail:
                # bankDetail has been updated, let's bind the old value to prev_hf
                prev_bankDetail = bankDetails.get(id=bankDetail_id)
                prev_bankDetail.health_facility_id = prev_hf_id
                prev_bankDetail.validity_to=TimeUtils.now()
                prev_bankDetail.save()
                # ... and create a new one with the new values
                bankMain = Bank.objects.get(BankId=bankDetail['bank_id'])
                bankbranche = BankBranch.objects.get(BranchId=bankDetail['branch_id'])
                bankDetail['bank_id'] = bankMain
                bankDetail['branch_id'] = bankbranche
                # bankDetail['account_name'] = user.id_for_audit
                # bankDetail['account_number'] = user.id_for_audit
                bankDetail['health_facility_id'] = hf_id
                bankDetail['validity_from'] = TimeUtils.now()
                HFBankDetails.objects.create(**bankDetail)
        else:
            bankMain = Bank.objects.get(BankId=bankDetail['bank_id'])
            bankbranche = BankBranch.objects.get(BranchId=bankDetail['branch_id'])
            bankDetail['bank_id'] = bankMain
            ...
            bankDetail['validity_from'] = TimeUtils.now()
            HFBankDetails.objects.create(**bankDetail)
    print("finale",prev_bankDetails)
    if prev_bankDetails:
        bankDetails.filter(id__in=prev_bankDetails).update(
            health_facility_id=prev_hf_id,
            validity_to=TimeUtils.now())
...
def update_or_create_health_facility(data, user):
    ...
    bankDetails = data.pop('bankDetails') if 'bankDetails' in data else []
    print("abc",data)
    ...
    process_bankDetails(user, bankDetails, prev_hf_id, hf.id, hf.bank_details)
    hf.save()
